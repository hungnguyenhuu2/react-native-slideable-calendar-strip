...
class DateItem extends PureComponent {
  render() {
    ...
    const dateItem = format(item, 'YYYY-MM-DD');
    ...
    let highlightBgColor = '#0066B3';
    let normalBgColor = 'white';
    const hightlightTextColor = '#fff';
    let normalTextColor = 'rgba(0,0,0,0.9)';
    let bgColor = '#6D88E6';
    if(new Date(selectDate)< TODAY){
      highlightBgColor = '#979797';
      bgColor = '#979797';
    }
    ...
    if(dateItem == dateToday ){
      normalBgColor = '#EFF8FF';
      normalTextColor= '#0066B3';
    }
    if(dateItem < dateToday ){
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!this.currentPageDatesIncludes(nextSelectedDate)) {
      ...
      if (this.state.datas.find(sameDay)) {
        ...
        //HungNH comment đoạn scroll page khi chọn ngày
        ...
        if (isFuture(nextSelectedDate)) {
          ...
          //HungNH fix đoạn tạo ngày + thêm 1 tuần để swipe
          let endDate = new Date(nextSelectedDate);
          let date = (endDate.getTime()/1000) + 7*86400;
          var dateTam = new Date(date * 1000)
          const nextDate = format(dateTam, 'YYYY-MM-DD');
          // const tail = endOfWeek(nextSelectedDate, { weekStartsOn: nextProps.weekStartsOn });
          //end đoạn cộng thêm ngày
          const tail = endOfWeek(nextDate, { weekStartsOn: nextProps.weekStartsOn });
